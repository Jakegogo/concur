...
/**
 * 集合编码器 Created by Jake on 2015/2/25.
 */
public class CollectionSerializer implements Serializer, Opcodes {
	...
	public void compile(Type type, MethodVisitor mv,
		...
		mv.visitMethodInsn(INVOKEINTERFACE, "java/util/Collection", "size",
				"()I", true);
		...
		if (type instanceof ParameterizedType) {
			elementType = TypeUtils.getParameterizedType(
		...
		if (elementClass == null || elementClass == Object.class
			...
			// 执行属性预编译
			MethodVisitor methodVisitor = context.invokeNextSerialize(elementType, null, mv);

			if (methodVisitor != null) {
                Serializer fieldSerializer = TransferConfig
                    .getSerializer(elementClass);
