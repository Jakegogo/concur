...
public class MapSerializer implements Serializer, Opcodes {
	...
	public void compile(Type type, MethodVisitor mv,
        ...
        Type keyType = TypeUtils.getParameterizedType(type, 0);
		Class<?> keyClass = TypeUtils.getRawClass(keyType);

		Type valueType = TypeUtils.getParameterizedType(type, 1);
		Class<?> valueClass = TypeUtils.getRawClass(valueType);
		...
		if (keyClass == null || keyClass == Object.class
			...
			// 执行属性预编译
			MethodVisitor methodVisitor = context.invokeNextSerialize(keyType, null, mv);
		...
		if (valueClass == null || valueClass == Object.class
			...
			// 执行属性预编译
			MethodVisitor methodVisitor = context.invokeNextSerialize(valueType, null, mv);
