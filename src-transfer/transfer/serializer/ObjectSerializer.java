	...
	public void compile(Type type, MethodVisitor mv,
		...
		for (FieldInfo fieldInfo : classInfo.getFieldInfos()) {
			...
			if (fieldType == null
				...
				// 执行属性预编译
				MethodVisitor nextMethodVisitor = context.invokeNextSerialize(fieldInfo.getType(),
						fieldInfo.getFieldName(), mv);

				if (nextMethodVisitor != null) {
                    fieldSerializer
                        .compile(fieldInfo.getType(), nextMethodVisitor, context);
                }
			}
